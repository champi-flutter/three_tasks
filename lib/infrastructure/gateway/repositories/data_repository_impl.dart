...
import 'package:three_tasks/infrastructure/gateway/data_source_interface/data_source.dart';
...
class DataRepositoryImpl
  ...
  async {
    try {
      ...
      final Result<List<FWeeklyTask>, Exception> result =
      ...
      switch (result) {
        ...
        case Success(value: final List<FWeeklyTask> resultValueList):
          ...
          for (final FWeeklyTask fTask in resultValueList) {
            ...
            // ID と ETask のセット
            final DataEntry<int, EWeeklyTask> dataEntry = DataEntry(
              id: fTask.id,
              value: ToETask.toEWeeklyTask<FWeeklyTask>(fTask),
            );
  ...
  // 折りたたみ用
  async {
    // 各パラメータペアを適用して DTask のリストに変換する
    final List<STask> newTaskList = updateParameterList.toSTaskList();
    ...
    switch (result) {
      case Success():
        // 日単位の値
        final Map<Date, List<SDailyTask>> resultDay = {};
        // 週単位の値
        final List<SWeeklyTask> resultWeek = [];
  ...
  ///
  /// 変更の反映の完了まで await する。
  Future<void> _cacheChanges(List<STask> newTaskList) async {
