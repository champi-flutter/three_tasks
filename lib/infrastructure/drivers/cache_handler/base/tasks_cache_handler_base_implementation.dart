...
abstract class TasksCacheHandlerBaseImplementation<K, Task extends CTask>
    extends LfuFloatingListCacheHandler<K, int, Task>
    implements TasksCacheHandlerBaseInterface<K, Task> {
  /// `super.capacity` を親に渡すための内部的なコンストラクタ
  /// （抽象クラスなので呼び出し不可）
  TasksCacheHandlerBaseImplementation({required super.capacity});
...
///
/// 親クラスの静的解析ツールの機能を引き継ぐためにインターフェースとして設置する。
abstract class WeeklyTasksCacheHandlerBaseImplementation
    extends LfuFloatingListCacheHandler<Date, UniqueWeek, List<CWeeklyTask>>
    implements WeeklyTasksCacheHandlerBaseInterface {
  /// `super.capacity` を親に渡すための内部的なコンストラクタ
  /// （抽象クラスなので呼び出し不可）
  WeeklyTasksCacheHandlerBaseImplementation(): super(capacity: 14);
