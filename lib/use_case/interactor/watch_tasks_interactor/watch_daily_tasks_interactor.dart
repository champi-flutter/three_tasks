...
class WatchDailyTasksInteractor implements WatchDailyTasksUseCase {
  ...
  Future<void> _startWatching(List<Date> dateList) =>
      _loader.loadAsync(() async {
        try {
          ...
          switch (result) {
            case Success(
                value: final Map<Date, List<DDailyTask>> existingValue
              ):
              ...
              if (emptyDateList.isNotEmpty) {
                // 新しい日付の枠（空のタスク）を作る
                final Result<Map<Date, List<DDailyTask>>, Exception>
                    newDateData = await _repository.createDailyTaskRecord(
  ...
  ///   2. 変換後のデータを反映させる
  ///
  Future<void> _onData(Map<Date, List<DDailyTask>> newDataMap) =>
      _loader.loadAsync(() async {
        try {
          // データの型を変換する
          final Map<Date, List<VDailyTask>> convertedMap =
              _convertToV(newDataMap);
          // 変換後のデータを反映させる
  ...
  void _publishDailyTasks(Map<Date, List<VDailyTask>> newDataMap) {
    _dailyTasksPresenter.handleDailyTasksUpdating(newDataMap);
  }
}
