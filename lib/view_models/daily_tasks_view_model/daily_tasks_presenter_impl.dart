...
class DailyTasksPresenterImpl implements DailyTasksPresenter {
  ...
  async {
    ...
    if (newDataOnToday != null) {
      // 当日のタスクの表示を更新する
      _todaysTasksVM.update(_convertToVList(newDataOnToday));
    }
    // 翌日のデータを含んでいる場合
    else if (newDataOnTomorrow != null) {
      // 翌日タスクの表示を更新する
      _tomorrowsTasksVM.update(_convertToVList(newDataOnTomorrow));
    }
  }

  /// データの型をエンティティから View State へ変換するプライベートメソッド
  List<VDailyTask> _convertToVList(List<EDailyTask> eTaskList) =>
      eTaskList.map<VDailyTask>(ToVTask.toVDailyTask<EDailyTask>).toList();
}
